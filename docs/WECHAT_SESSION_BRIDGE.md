# 微信公众号任意文章：本地微信会话桥（旧兼容模块）

## 结论

本模块因需要真实微信客户端，已按当前风险要求退出生产主路径；默认配置不会启动或调用它。
当前主路径见 [`WECHAT_COOKIE_PROTOCOL.md`](WECHAT_COOKIE_PROTOCOL.md)。以下内容只保留
旧部署的兼容说明。

该方案不是“匿名接口”，而是把已登录微信客户端当作短时会话提供者。微信桌面端打开
...
- `show_comment=0`：只表示当前页面未展示评论区，不能据此判断作者关闭；只有评论接口
  成功响应中的 `enabled=0` 才是完整空结果。
