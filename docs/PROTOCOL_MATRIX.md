...
持久化会话接管，不接入付费数据供应商。快手游客样本可用；小红书首屏曾在新鲜笔记成功，
但本轮当前样本失败，不能承诺匿名稳定性。公众号任意文章互动/评论及小红书深分页不存在
...
| 小红书 | 互动量 | 无会话无法稳定获取 | 游客 SSR 曾成功；自有会话可用 `xhshow` | 本轮当前样本协议及浏览器均失败；旧链接/令牌和游客风控影响明显 |
| 小红书 | 评论 | 游客首屏不稳定 | 游客浏览器曾截获 `/api/sns/web/v2/comment/page` | 本轮当前样本失败；纯算 `XYS_/XYW_` 匿名直连仍返回 HTTP 406 |
...
快手互动量/一级评论。小红书本轮两项均未通过，不把历史新鲜样本成功冒充当前稳定能力；
