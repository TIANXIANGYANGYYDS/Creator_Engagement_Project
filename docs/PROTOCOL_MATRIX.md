...
最后验证：2026-08-25，环境：`MyAgent` / Python 3.13.12。当前生产不执行人工操作、扫码或
浏览器登录；不接入付费数据供应商。快手企业协议链路最新全量为互动/评论均
...
签名评论端点，不能合并为一次上游请求，也没有接入付费接口。公众号已区分 Cookie-only、自有公众号
...
历史登录/Profile 工具仅保留兼容，当前生产不得调用。唯一账号会话例外是部署方通过
`XIAOHONGSHU_SESSION_MODE=cookie` 显式注入的小红书专用 Cookie。
