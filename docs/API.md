...
前三类公开分享 URL 可匿名读取页面公开互动量。最后一类客户端跳转 URL 不公开视频数据；配置
`WECHAT_CHANNELS_MIDU_URL` 后，互动接口可按 URL 查询蜜度近 30 天已收录的互动总量，无需微信
账号。评论正文仍必须使用授权 Windows 微信侧车。两个数据源均不可用时单项接口返回 HTTP
502，批量项为 `failed`，不会把空值或公开校验失败伪装为成功。
...
代理成本。它不包含蜜度服务的合同费用或接口配额，也不是对客户的商业收费金额。
...
| 微信视频号 | 取决于数据源 | 支持 | 支持 | 支持 | 公开页支持 | 不支持 | 不支持 | 蜜度可返回 | 不支持 | 授权侧车可用 |
| 小红书 | 不支持 | 支持 | 支持 | 支持 | 支持 | 不支持 | 不支持 | 不支持 | 不支持 | Cookie 模式可用 |
...
| 微信视频号 | 公开分享页返回点赞、评论数、分享、收藏；客户端 `feedID` 可由蜜度返回已收录点赞、评论数和转发；授权侧车详情还可返回平台会话可见播放量 |
